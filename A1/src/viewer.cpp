...
bool Viewer::updateGame() {
  // std::cout << "Tick motherfucker. mCurrentTick = " << mCurrentTick << " mCurrentDifficulty = " << mCurrentDifficulty << std::endl;
  if (mCurrentTick >= mCurrentDifficulty) {
    // if (mGame.tick() >= 0) {
    //   on_expose_event(NULL);
    // }
    // std::cout << "Game Tick motherfucker." << std::endl;

    mGame.tick();
    mCurrentTick %= mCurrentDifficulty;
  }
  if (!mIsScaleUsed) {
...
void Viewer::changeDifficulty( Difficulty diff ) { 
  mCurrentDifficulty = diff; 
  mCurrentTick %= mCurrentDifficulty;
}
...
bool Viewer::on_expose_event(GdkEventExpose* event)
{
  ...
  drawCube(leftWall, GL_QUADS, NORMAL_FACE, false, Point3D(1.0, 20.0, 1.0));

  Point3D bottomWall(-1.0, -1.0, 0.0);
  drawCube(bottomWall, GL_QUADS, NORMAL_FACE, false, Point3D(12.0, 1.0, 1.0));

  Point3D rightWall(10.0, 0.0, 0.0);
  drawCube(rightWall, GL_QUADS, NORMAL_FACE, false, Point3D(1.0, 20.0, 1.0));
  ...
  if (mMode != MODE_WIREFRAME) {
    for (int i = 0; i < mGame.getWidth(); ++i) {
      for (int j = 0; j < mGame.getHeight() + 4; ++j) {
        ...
        if (piece != -1) {
          Point3D cube(i, j, 0);
          bool isMulticoloured = (mMode == MODE_MULTICOLOURED) ? true : false;
          drawCube(cube, GL_QUADS, RED_FACE, isMulticoloured);
        }
      }
    } 
  }
...
void Viewer::drawCube(Point3D position, GLenum type, Colour colour, bool isMulticoloured, Point3D scale) {
  glPushMatrix();
  ...
  drawFace(CUBE[0], CUBE[1], CUBE[4], CUBE[2], type, (isMulticoloured) ? MULTICOLOURED_FACE[0] : colour);
  drawFace(CUBE[2], CUBE[5], CUBE[7], CUBE[4], type, (isMulticoloured) ? MULTICOLOURED_FACE[1] : colour);
  drawFace(CUBE[5], CUBE[7], CUBE[6], CUBE[3], type, (isMulticoloured) ? MULTICOLOURED_FACE[2] : colour);
  drawFace(CUBE[0], CUBE[1], CUBE[6], CUBE[3], type, (isMulticoloured) ? MULTICOLOURED_FACE[3] : colour);
  drawFace(CUBE[0], CUBE[2], CUBE[5], CUBE[3], type, (isMulticoloured) ? MULTICOLOURED_FACE[4] : colour);
  drawFace(CUBE[4], CUBE[7], CUBE[6], CUBE[1], type, (isMulticoloured) ? MULTICOLOURED_FACE[5] : colour);
...
bool Viewer::on_button_release_event(GdkEventButton* event)
{
  ...
  mCurrentMousePos = Point2D(event->x, event->y);
  if (mPersistenceValue > 2 || mPersistenceValue < -2) {
    // mPersistenceValue = 0.0;
...
bool Viewer::on_motion_notify_event(GdkEventMotion* event)
{
  ...
  mCurrentMousePos = Point2D(event->x, event->y);
  double dist = distance(mCurrentMousePos, mLastMousePos) * 2;
